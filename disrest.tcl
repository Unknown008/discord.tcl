...
proc discord::rest::Send { token verb resource {body {}} {cmd {}} args } {
  ...
  if {[regexp {^(/(?:channels|guilds)/\d+)} $resource -> route]} {
    ...
    if {$sendCount == 0} {
      after [expr {($BurstLimitPeriod+1) * 1000}] [list \
    ...
    if {[dict exists $RateLimits $token $route X-RateLimit-Remaining]} {
      ...
      if {$remaining <= 0} {
        ...
        if {$secsRemain >= -3} {
          set msg "Internal: Send Rate-limited on $route, reset in $secsRemain"
          append msg " seconds"
          ${log}::warn $msg
          return
        }
...
proc discord::rest::SendCallback { sendId token } {
  ...
  switch $status {
    ok {
      ...
      set rates {X-RateLimit-Limit X-RateLimit-Remaining X-RateLimit-Reset}
      foreach header $rates {
        if {[info exists route] && [info exists meta($header)]} {
          dict set RateLimits $discordToken $route $header $meta($header)
        }
      }
      ...
      if {$ncode >= 300} {
        ${log}::warn [join [list \
          "SendCallback${sendId}: $url: $code:" [::http::data $token]]]
        if {[llength $cmd] > 0} {
    ...
    switch $type {
      ...
      array {
        if {$field eq "fields" && [llength $value] > 25} {
          return -code error "Number of fields in embed cannot exceed 25."
        }
